  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-sm font-bold text-gray-500">Retrieving full itinerary details...</p>
      </div>
    );
  }
  ...
  if (!order) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center">
        <p className="font-black text-xl text-gray-800">Itinerary Not Found</p>
      ...
      <div className="max-w-6xl mx-auto px-4 pt-6 space-y-6 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-2 border-b border-gray-200">
          <h1 className="text-3xl font-black text-gray-900 tracking-tight flex items-center gap-3">
            Flight PNR: {pnr} 
            {isTestMode && <span className="bg-yellow-100 text-yellow-800 text-xs font-bold px-2 py-1 rounded-md uppercase tracking-widest">Test</span>}
          </h1>
          <div className="flex flex-wrap items-center gap-3">
            <button className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-bold text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">
              <Ticket size={16} /> Ticket numbers
            </button>
            <button className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-bold text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">
              <Download size={16} /> Export itinerary
            </button>
            <button className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-bold text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">
              <Settings size={16} /> Manage this order
            </button>
          </div>
        </div>
        ...
        {isTestMode && (
          <div className="bg-[#FFFBEB] border border-[#FEF3C7] p-4 rounded-xl flex items-center gap-3 text-[#B45309]">
            ...
            <p className="text-sm font-medium"><strong>You are in test mode:</strong> No live orders will be created, nor money change hands, while test mode is enabled</p>
          </div>
        )}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200 bg-gray-50/50">
                <h2 className="text-lg font-bold text-gray-900">Journey details</h2>
              </div>
                ...
                {order.slices?.map((slice: any, sIdx: number) => {
                  ...
                  return (
                    <div key={sIdx} className="space-y-6">
                      ...
                      <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-sm font-medium text-gray-900 w-full">
                        ...
                        <div className="ml-auto flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-gray-200 shadow-sm">
                          ...
                          <span className="font-black text-gray-900 text-xs tracking-wide">{carrier.name}</span>
                        </div>
                      </div>
                      ...
                      <div className="relative pl-6 border-l-[3px] border-gray-200 space-y-6 ml-2">
                        <div className="relative">
                          ...
                          <p className="text-sm font-bold text-gray-900">{formatFullDate(firstSeg.departing_at)}, {formatTime(firstSeg.departing_at)}</p>
                          <p className="text-sm text-gray-600 mt-1">Depart from {firstSeg.origin.name} ({firstSeg.origin.iata_code}){firstSeg.origin_terminal ? `, Terminal ${firstSeg.origin_terminal}` : ''}</p>
                        </div>

                        <div className="py-2">
                          <p className="text-sm text-gray-500 font-medium">Flight duration: {formatDuration(firstSeg.duration)}</p>
                        ...
                        <div className="relative">
                          <div className="absolute -left-[32px] top-1 w-4 h-4 bg-white border-[3px] border-gray-400 rounded-full"></div>
                          <p className="text-sm font-bold text-gray-900">{formatFullDate(lastSeg.arriving_at)}, {formatTime(lastSeg.arriving_at)}</p>
                          <p className="text-sm text-gray-600 mt-1">Arrive at {lastSeg.destination.name} ({lastSeg.destination.iata_code}){lastSeg.destination_terminal ? `, Terminal ${lastSeg.destination_terminal}` : ''}</p>
                        </div>
                      </div>
                    </div>
                  );
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 bg-gray-50/50">
                  <h3 className="text-sm font-bold text-gray-900">Order change policy</h3>
                </div>
                <div className="p-6">
                  <div className={`flex items-start gap-3 p-4 rounded-xl border ${changeCond?.allowed ? 'bg-emerald-50 border-emerald-100 text-emerald-800' : 'bg-gray-50 border-gray-200 text-gray-600'}`}>
                    {changeCond?.allowed ? <CheckCircle2 size={20} className="text-emerald-600 shrink-0" /> : <XCircle size={20} className="text-gray-400 shrink-0" />}
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 bg-gray-50/50">
                  <h3 className="text-sm font-bold text-gray-900">Order refund policy</h3>
                </div>
                <div className="p-6">
                   <div className={`flex items-start gap-3 p-4 rounded-xl border ${refundCond?.allowed ? 'bg-emerald-50 border-emerald-100 text-emerald-800' : 'bg-gray-50 border-gray-200 text-gray-600'}`}>
                    {refundCond?.allowed ? <CheckCircle2 size={20} className="text-emerald-600 shrink-0" /> : <XCircle size={20} className="text-gray-400 shrink-0" />}
                    <p className="text-sm font-bold leading-relaxed">
                      {refundCond?.allowed 
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200 bg-gray-50/50">
                <h2 className="text-lg font-bold text-gray-900">Passengers</h2>
              </div>
                    ...
                    <div key={p.id} className={`${idx !== 0 ? 'pt-8 border-t border-gray-200' : ''}`}>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4 mb-6">
                        <div>
                          ...
                          <p className="text-sm font-medium text-gray-900">{formatTitle(p.title)} {p.given_name} {p.family_name}</p>
                        </div>
                        <div>
                          <p className="text-xs font-bold text-gray-500 uppercase mb-1">E-mail</p>
                          <p className="text-sm font-medium text-gray-900 break-words">{p.email || 'Not provided'}</p>
                        </div>
                        <div>
                          <p className="text-xs font-bold text-gray-500 uppercase mb-1">Date of birth</p>
                          <p className="text-sm font-medium text-gray-900">{formatDOB(p.born_on)}</p>
                        </div>
                        <div>
                          <p className="text-xs font-bold text-gray-500 uppercase mb-1">Gender</p>
                          <p className="text-sm font-medium text-gray-900 capitalize">{p.gender === 'm' ? 'Male' : p.gender === 'f' ? 'Female' : p.gender}</p>
                        </div>
                        <div>
                      ...
                      <div>
                        <p className="text-xs font-bold text-gray-500 uppercase mb-2">Flight information</p>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-xs font-bold bg-emerald-100 text-emerald-800">
                      Confirmed
                    </span>
                  </div>
                  <div>
                    <p className="text-xs font-bold text-gray-500 uppercase mb-1">Airline</p>
                    <p className="text-sm font-medium text-gray-900">{airlineOwner}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="p-6">
                <div className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className="w-8 h-8 rounded-full bg-theme-primary text-white flex items-center justify-center font-bold text-sm shrink-0">
        ...
        <div className="pt-8 mt-8 border-t border-gray-200 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl p-6 border border-dashed border-gray-300 flex flex-col justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Need a place to stay in {destIata}? Add it to your itinerary.</p>
            </div>
            <Link href={`/?tab=stays&dest=${destIata}`} className="self-start px-4 py-2 bg-blue-50 text-blue-700 font-bold text-xs uppercase tracking-widest rounded-lg hover:bg-blue-100 transition-colors">
              Add a Hotel
            </Link>
          </div>
          <div className="bg-white rounded-xl p-6 border border-dashed border-gray-300 flex flex-col justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Discover top-rated tours and attractions for your trip.</p>
            </div>
            <Link href={`/?tab=tours&dest=${destIata}`} className="self-start px-4 py-2 bg-amber-50 text-amber-700 font-bold text-xs uppercase tracking-widest rounded-lg hover:bg-amber-100 transition-colors">
              Browse Tours
            </Link>
          </div>
        </div>
