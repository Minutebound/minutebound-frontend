...
import { Search, Loader2, Calendar, TrendingUp, PenBoxIcon, Map, X, Users, Plane, Car, DollarSign, GemIcon, ChevronUp, ChevronDown } from "lucide-react";
...
export default function SearchBar({
    ...
    <div className="relative w-full z-30 flex flex-col items-center justify-center">
      {isCompact && (
        <button 
          ...
          className="absolute top-4 right-4 md:top-6 md:right-6 w-9 h-9 sm:w-10 sm:h-10 bg-theme-white/10 border border-theme-white/20 text-theme-white rounded-full flex items-center justify-center hover:bg-theme-white/20 transition-all z-[10001] active:scale-90 backdrop-blur-md cursor-pointer"
        >
          <X size={24} />
        </button>
      )}
      ...
      <div className={`relative z-10 mx-auto px-6 md:px-6 lg:px-8 py-8 lg:py-12 overflow-visible ${isCompact ? 'w-full max-w-[100%] pt-8 lg:pt-8' : 'w-[85%] max-w-[1800px]'}`}>        
        
        {/* Travel Mode Pills */}
        <div className="mb-4 flex flex-wrap items-center gap-4 relative z-10">
        ...
        {/* MAIN INPUT BOX */}
        <div className="bg-theme-white w-full rounded-[2rem] shadow-[0_16px_50px_rgba(0,0,0,0.12)] p-4 md:p-5 lg:p-6 flex flex-col gap-4 relative z-[90]">
          ...
          <div className="flex">
            <div className="relative group cursor-pointer flex items-center">
              <select
                ...
                className="appearance-none text-[16px] bg-theme-primary/10 text-theme-primary uppercase tracking-widest cursor-pointer outline-none py-1 pl-3 pr-8 rounded-lg border border-theme-primary/20 hover:bg-theme-primary/20 transition-all"
              >
                <option value="round-trip">Round Trip</option>
                <option value="one-way">One Way</option>
              </select>
              <ChevronDown size={10} className="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none text-theme-primary" />
            </div>
          </div>

          {/* FIXED: items-start for mobile so it doesn't break, lg:items-end to align bottoms on desktop */}
          <div className="flex flex-col lg:flex-row items-start lg:items-end w-full gap-4 lg:gap-3 overflow-visible relative z-[100]">
            
            {/* Origin & Destination */}
            <div className="flex flex-col flex-[1.2] w-full relative z-[100]">
              <div className="flex w-full px-2 lg:px-4 mb-1.5 items-center">
                 <div className="flex-1 flex items-center gap-3">
                     <label className="text-[16px] font-black uppercase tracking-widest text-theme-secondary/60">From</label>
                 </div>
                 <label className="text-[16px] font-black flex-1 uppercase tracking-widest text-theme-secondary/60 pl-4 lg:pl-8">To</label>
              </div>

              <div className="relative flex flex-row h-12 lg:h-14 bg-theme-white rounded-[1rem] lg:rounded-l-[1rem] border-[1.5px] border-theme-secondary/30 focus-within:border-theme-primary/50 transition-colors shadow-sm group z-[100]">
                <div className="flex-1 relative flex items-center px-3 md:px-5 lg:px-5 rounded-l-[1rem] lg:rounded-l-[1rem] hover:bg-theme-secondary/5 transition-colors border-r border-theme-secondary/20 z-[60] focus-within:z-[100]">
            ...
            {/* Dates Container */}
            <div className="flex flex-col flex-[1] w-full relative z-[90]">
              <div className="flex w-full px-2 lg:px-4 mb-1.5 items-center">
                 <label className="flex-1 uppercase text-[16px] font-black tracking-widest text-theme-secondary/60">Depart</label>
                 <label className={`flex-1 uppercase text-[16px] font-black tracking-widest text-theme-secondary/60 pl-2 lg:pl-4 ${tripType === 'one-way' ? 'opacity-30' : ''}`}>Return</label>
              </div>

              <div className="relative flex flex-row h-12 lg:h-14 bg-theme-white rounded-[1rem] lg:rounded-l-[1rem] border-[1.5px] border-theme-secondary/30 focus-within:border-theme-primary/50 transition-colors shadow-sm z-[90]">
                
                <div className="flex-1 relative flex items-center px-3 md:px-5 lg:px-5 rounded-l-[1rem] lg:rounded-l-[1rem] hover:bg-theme-secondary/5 transition-colors border-r border-theme-secondary/20 z-[60] focus-within:z-[100]">
                  <Calendar size={16} className="text-theme-primary/80 shrink-0 mr-1.5" />
                  <DatePicker
                    ...
                    className="w-full bg-transparent font-bold  md: text-theme-secondary outline-none border-none cursor-pointer placeholder-theme-secondary/40"
                  />
                ...
                <div className={`flex-1 relative flex items-center px-3 md:px-5 lg:px-5 rounded-r-[1rem] lg:rounded-r-[1rem] transition-colors z-[60] focus-within:z-[100] ${tripType === 'one-way' ? 'bg-theme-secondary/5 cursor-not-allowed' : 'hover:bg-theme-secondary/5'}`}>
                  <Calendar size={16} className={`shrink-0 mr-1.5 ${tripType === 'one-way' ? 'text-theme-secondary/30' : 'text-theme-primary/80'}`} />
                  <DatePicker
                    ...
                    className="w-full bg-transparent font-bold  md: text-theme-secondary outline-none border-none cursor-pointer placeholder-theme-secondary/40 disabled:cursor-not-allowed disabled:text-theme-secondary/40"
                  />
            ...
            {/* Search Button (Aligned naturally to the bottom by lg:items-end on the parent) */}
            <div className="w-full lg:w-auto shrink-0 relative z-10 mt-4 lg:mt-0">
              {!isWorking ? (
                <button
                  className="w-full lg:w-auto h-12 lg:h-14 lg:min-w-[140px] rounded-[1rem] lg:rounded-full bg-theme-primary text-theme-white font-black tracking-wider flex items-center justify-center gap-2 hover:brightness-110 transition-all px-8 shadow-lg active:scale-95 border-none"
                ...
                <div className="w-full lg:w-auto h-12 lg:h-14 lg:min-w-[140px] rounded-[1rem] lg:rounded-full bg-theme-primary/80 text-theme-white/80 font-black flex items-center justify-center gap-2 px-8 shadow-inner cursor-not-allowed border-none">
                  <Loader2 size={24} className="animate-spin text-theme-white" />
                </div>
              )}
            </div>
            ...
            <div className="flex flex-row flex-wrap sm:flex-nowrap items-center justify-between sm:justify-start gap-1 sm:gap-2 w-full xl:w-auto overflow-visible relative z-50">
              ...
              <div className="relative z-50 shrink-0" ref={travellerRef}>
                <button 
                  ...
                  className="flex items-center gap-1.5 sm:gap-2 font-bold text-[16px] text-theme-secondary hover:text-theme-primary transition-colors py-1 px-1.5 sm:px-2 rounded-lg hover:bg-theme-secondary/10 whitespace-nowrap"
                >
                  ...
                  <div className="absolute top-full left-0 mt-3 w-[260px] sm:w-72 bg-theme-white text-theme-secondary rounded-2xl shadow-xl border border-theme-secondary/20 p-5 z-[100] animate-in fade-in zoom-in-95 duration-200">                    
                    <div className="flex items-center justify-between mb-5">
                      <div>
                        ...
                        <div className="text-[10px] text-theme-muted uppercase tracking-wider">Ages 12+</div>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-[10px] text-theme-muted uppercase tracking-wider">Ages 2-11</div>
                      </div>
              ...
              <div className="relative group cursor-pointer flex items-center gap-1 sm:gap-1.5 py-1 px-1.5 sm:px-2 text-[16px] rounded-lg hover:bg-theme-secondary/10 transition-colors whitespace-nowrap shrink-0">
                <span className="text-base text-theme-primary/80">{budget === 'budget' ? <DollarSign size={16}/> : <GemIcon size={16}/>}</span>
                <select 
                  value={budget} 
                  onChange={(e) => setBudget(e.target.value as "budget" | "Premium")}
                  className="appearance-none bg-transparent font-bold text-theme-secondary cursor-pointer outline-none pr-4 sm:pr-5 hover:text-theme-primary transition-colors border-none"
                >
  ...
  <span className="text-[16px] font-bold tracking-wider text-theme-secondary hidden sm:inline">Radius:</span>
  <span className="text-[16px] font-bold tracking-wider text-theme-secondary sm:hidden">Rad:</span>
  
  {/* Custom Input Wrapper with Arrows */}
  <div className="flex items-center bg-theme-secondary/5 border border-theme-secondary/20 rounded-md focus-within:border-theme-primary focus-within:ring-1 focus-within:ring-theme-primary transition-all overflow-hidden">
    <input
      type="number"
      min={1} 
      max={100}
      ...
      // Removed border/ring from input directly, moved to wrapper
      className="w-[36px] sm:w-[46px] bg-transparent py-1 font-bold text-theme-secondary outline-none text-center hide-arrows"
    />
    
    {/* Upper and Lower Arrows */}
    <div className="flex flex-col">
      <button
        type="button"
        onClick={() => setRadius(Math.min(100, radius + 1))}
        className="flex items-center justify-center px-1.5 py-[1px]"
      >
        <ChevronUp size={10} className="text-theme-secondary cursor-pointer" />
      </button>
      <button
        type="button"
        ...
        className="flex items-center justify-center px-1.5 py-[1px] "
      >
        <ChevronDown size={10} className="text-theme-secondary cursor-pointer" />
      </button>
    </div>
  </div>
  </div>

  <span className="text-[16px] font-bold uppercase tracking-wider text-theme-secondary">mi</span>
</div>
            {topDestinations.length > 0 && (
              <div className="flex flex-row items-center w-full xl:w-auto relative z-10 py-1">
                <div className="flex items-center gap-1.5 shrink-0 pr-3">
                   ...
                   <span className="text-[10px] font-black uppercase text-theme-secondary/60 tracking-wider">Trending:</span>
                </div>
                <div className="flex flex-row items-center gap-2 overflow-x-auto no-scrollbar w-full">
                  {topDestinations.map((dest, idx) => (
                    <button
                      ...
                      className="px-4 py-1.5 rounded-full bg-theme-secondary/5 text-theme-secondary/80 text-[10px] font-bold hover:bg-theme-secondary/10 transition-all whitespace-nowrap border border-theme-secondary/10 shadow-sm shrink-0"
                    >
                      {dest.city}
                    </button>
                  ))}
      ...
      {/* GLOBAL CSS FOR REACT-DATEPICKER Z-INDEX */}
 ...
 return (
    <>
      {isCompact && (
        <div className="w-full bg-theme-white/50 backdrop-blur-xl py-3 px-4 md:px-6 flex items-center justify-around border-b border-theme-secondary/20 z-20 sticky top-0 shadow-sm transition-all duration-300">
          
          <div className="flex items-center">
            
            <button
              onClick={() => setIsOverlayOpen(true)}
              className="w-full bg-theme-white border border-theme-secondary/20 shadow-[0_2px_10px_rgba(0,0,0,0.04)] hover:shadow-[0_4px_16px_rgba(0,0,0,0.08)] rounded-full flex items-center p-1.5 sm:p-2 transition-all duration-300 cursor-pointer group"
            >
              <div className="flex flex-col sm:hidden flex-1 text-left overflow-hidden pl-4 py-1">
                <span className="font-bold  text-theme-secondary truncate">
                  {source || 'Anywhere'} {destination ? `to ${destination}` : ''}
                </span>
                <div className="flex items-center text-[16px] text-theme-secondary/60 gap-1 mt-[2px] truncate font-medium">
              ...
              <div className="hidden sm:flex items-center justify-between flex-1 pl-2 pr-1">
                <div className="flex items-center flex-1">
                  ...
                  <div className="font-medium  text-theme-secondary/70 px-4 py-2 rounded-full hover:bg-theme-secondary/5 transition-colors whitespace-nowrap">
                  ...
                  <div className="ml-2 flex items-center gap-1.5 uppercase tracking-widest rounded-full text-[10px] text-theme-secondary font-black bg-theme-primary/10 px-2 py-1 rounded-md shrink-0">
                    {travelMode === 'fly' ? <Plane size={16}/> : <Car size={16}/>}
            ...
            {onMapToggle && (
              ...
              >
                {mapOpen ? <X size={16} className="text-theme-secondary" /> : <Map size={16} className="text-theme-secondary" />}
              </button>
            )}
      ...
      {isCompact && isOverlayOpen && (
        // OVERLAY BACKGROUND: Boosted to z-[9999] to clear all page content
        <div className="fixed inset-0 z-[9999] flex items-start justify-center pt-20 px-4 bg-black/60 backdrop-blur-sm transition-all duration-300">
          
          {/* Click-away backdrop */}
          <div className="absolute inset-0 cursor-pointer" onClick={() => setIsOverlayOpen(false)}></div>
          
          {/* POPUP CONTAINER */}
          <div className="relative max-w-full lg:max-w-[70%] animate-in slide-in-from-top-4 fade-in duration-200 z-[10000]">
            <div className="max-w-full bg-transparent rounded-[2.5rem] shadow-2xl overflow-visible relative border-none z-[10000]">
  ...
  return (
    <div className="flex items-center gap-3 bg-theme-secondary/5 rounded-full p-1 border border-theme-secondary/20">
      <button 
        ...
        disabled={value <= min}
      >−</button>
      <span className="font-black  text-theme-secondary text-center w-4">{value}</span>
      <button 
        type="button" 
        className="w-8 h-8 rounded-full bg-theme-white shadow-sm text-theme-secondary hover:text-theme-primary flex items-center justify-center font-bold text-lg disabled:opacity-40 transition-all active:scale-90" 
