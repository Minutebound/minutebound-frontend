            ...
            <div className="p-4 sm:p-6 flex flex-col lg:flex-row lg:items-center gap-4 lg:gap-8 relative">
              ...
              <div className="flex-1 w-full flex flex-col">
                    ...
                    <div key={i} className="flex items-center gap-2 sm:gap-4">
                      <div className="flex-1 flex items-center gap-2 sm:gap-4">
                        ...
                        <div className="flex-1 flex flex-col items-center gap-1 px-1 sm:px-2">
                           <div className="w-10 text-[16px] align-center font-bold text-theme-secondary/50">{i === 0 ? 'DEPART' : 'RETURN'}</div>
                          ...
                          <span className={`text-[16px] font-bold ${itin.stops === 0 ? 'text-theme-success' : 'text-theme-light-gray'} tracking-tighter text-center line-clamp-1`}>
                ...
                <div className="flex flex-wrap items-center gap-2 pt-4 mt-4 border-t border-theme-secondary/5">
                  {flight.carbon_emissions_kg && (
                    <span className="flex items-center gap-1 px-2 py-1 bg-emerald-50 text-emerald-600 rounded-md text-[10px] font-black uppercase tracking-widest border border-emerald-100">
                      <Leaf size={12} /> {flight.carbon_emissions_kg} kg CO₂
                    </span>
                  )}
                  {hasWifi && (
                    <span className="flex items-center gap-1 px-2 py-1 bg-theme-secondary/5 text-theme-secondary/60 rounded-md text-[10px] font-black uppercase tracking-widest border border-theme-secondary/10">
                      <Wifi size={12} /> WiFi
                    </span>
                  )}
                  {hasPower && (
                  ...
                  {(totalCheckedBags > 0 || totalCarryOnBags > 0) && (
                    <span className="flex items-center gap-1 px-2 py-1 bg-blue-50 text-blue-600 rounded-md text-[10px] font-black uppercase tracking-widest border border-blue-100">
              ...
              <div className="flex justify-center w-auto lg:w-auto py-2">
                <div className="flex flex-col items-center gap-1 group">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-theme-primary transition-colors">Details</span>
              ...
              <div className="flex flex-row lg:flex-col justify-between items-center lg:items-end gap-3 shrink-0 border-t lg:border-t-0 lg:border-l border-theme-secondary/10 pt-4 lg:pt-0 pl-0 lg:pl-6 w-full lg:w-auto">
                <div className="text-left lg:text-right">
                  <p className="text-[24px] font-black text-theme-secondary tracking-tighter leading-none"><span className="text-[16px] font-black pr-1 font-normal">{flight.currency || 'USD'} </span>{getPrice(flight).toFixed(0)}</p>
                  <p className="text-[10px] sm:text-[10px] uppercase text-theme-secondary/30 tracking-widest mt-1">
                            ...
                            return (
                              <React.Fragment key={sIdx}>
                                <div className="relative">
                                  <div className="absolute -left-[24px] top-1.5 w-2 h-2 rounded-full bg-theme-white/50 border-2 border-theme-primary" />
                                  <div className="flex flex-col bg-theme-white p-4 rounded-lg border border-theme-secondary/5 shadow-sm">
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                      <div className="flex flex-col">
                                        <span className="font-black text-theme-secondary text-[16px]">
                                          ...
                                          {seg.departure_terminal && <span className="text-theme-light-gray ml-1.5 bg-theme-primary/10 px-1.5 py-0.5 rounded-sm text-[10px] uppercase tracking-widest">TERMINAL-{seg.departure_terminal}</span>}
                                        </span>
                                        <span className="font-bold text-theme-secondary/60 text-[16px] mt-0.5">
                                      ...
                                      <div className="flex flex-col sm:text-right">
                                        <span className="font-black text-theme-secondary text-[16px]">
                                          ...
                                          {seg.arrival_terminal && <span className="text-theme-light-gray ml-1.5 bg-theme-primary/10 px-1.5 py-0.5 rounded-sm text-[10px] uppercase tracking-widest">TERMINAL-{seg.arrival_terminal}</span>}
                                        </span>
                                        <span className="font-bold text-theme-secondary/60 text-[10px] mt-0.5">
                                    ...
                                    {(aircraftName || seg.duration) && (
                                      <div className="mt-4 pt-3 border-t border-theme-secondary/5 flex flex-wrap gap-4 text-[10px] font-black uppercase tracking-widest text-theme-secondary/40">
                                ...
                                {nextSeg && (
                                  <div className="relative py-2 pl-2">
                                    <div className="flex items-center gap-3 text-theme-secondary/50 font-black text-[10px] uppercase tracking-widest bg-theme-secondary/5 w-fit px-4 py-2 rounded-md border border-theme-secondary/10">
                  ...
                  <div className="lg:col-span-4 space-y-6">
                    <div className="space-y-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="p-3 bg-theme-white rounded-sm border border-theme-secondary/5 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 font-black uppercase text-[10px] tracking-widest">
...
function BenefitIcon({ icon, label }: { icon: React.ReactNode, label: string }) {
  return (
    <div className="flex items-center gap-2 p-3 bg-theme-white rounded-sm border border-theme-secondary/5 shadow-sm overflow-hidden">
      ...
      <span className="font-black uppercase text-[10px] tracking-widest text-theme-secondary/60 truncate">{label}</span>
    </div>
  );
}