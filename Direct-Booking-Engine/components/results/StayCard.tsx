      ...
      <div className="p-4 sm:p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-4 lg:gap-6 relative">
        ...
        <div className="flex flex-row gap-4 flex-1 items-start lg:items-center">
          ...
          <div className="w-24 h-24 sm:w-28 sm:h-28 bg-theme-secondary/5 rounded-xl border border-theme-secondary/10 flex flex-col items-center justify-center shrink-0 text-theme-secondary/30">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-center px-2">No Image Provided</span>
          </div>
          ...
          <div className="flex flex-col gap-1.5 flex-1 pt-1">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {chainCode && (
                <span className="bg-theme-secondary/10 text-theme-secondary/80 px-2 py-0.5 rounded-[4px] text-[10px] font-black uppercase tracking-widest border border-theme-secondary/10">
                  Chain: {chainCode}
                </span>
              )}
            </div>
            ...
            {offer?.rooms?.[0]?.amenities && offer.rooms[0].amenities.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-2 max-w-md hidden sm:flex">
                {offer.rooms[0].amenities.slice(0, 4).map((amenity: string, idx: number) => (
                  <span key={idx} className="text-[10px] font-bold text-theme-secondary/60 bg-theme-secondary/5 px-2 py-1 rounded-[4px] capitalize border border-theme-secondary/5 whitespace-nowrap">
                    {amenity.replace(/_/g, ' ').toLowerCase()}
                  </span>
                ))}
                {offer.rooms[0].amenities.length > 4 && (
                  <span className="text-[10px] font-bold text-theme-secondary/40 px-1 py-1">
        ...
        <div className="flex flex-row lg:flex-col justify-between items-center lg:items-end gap-3 shrink-0 border-t lg:border-t-0 lg:border-l border-theme-secondary/10 pt-4 lg:pt-0 pl-0 lg:pl-6 w-full lg:w-auto">
            {!isUnavailable && offer ? (
              <div className="text-left lg:text-right">
                <p className="text-[24px] font-black text-theme-secondary tracking-tighter leading-none">
                  ${offer.price?.toFixed(0)}
                </p>
                <p className="text-[10px] sm:text-[10px] uppercase text-theme-secondary/40 font-bold tracking-widest mt-1.5">
              ...
              <div key={i} className="flex flex-col lg:flex-row justify-between bg-theme-white p-5 rounded-xl border border-theme-secondary/10 gap-6 items-start shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="flex flex-col gap-2 flex-1">
                  <div className="flex items-center justify-between">
                    ...
                    {/* Refundable Badge */}
                    {room.is_refundable ? (
                      <span className="bg-green-50 text-green-700 border border-green-200 px-2 py-1 rounded-[4px] text-[10px] font-black uppercase tracking-widest flex items-center gap-1">
                        <CheckCircle2 size={10} /> Refundable
                      </span>
                    ) : (
                      <span className="bg-theme-secondary/5 text-theme-secondary/50 border border-theme-secondary/10 px-2 py-1 rounded-[4px] text-[10px] font-black uppercase tracking-widest flex items-center gap-1">
                         Non-Refundable
                      </span>
                    )}
                  </div>
                  ...
                  {room.description && (
                    <p className="text-[16px] font-medium text-theme-secondary/70 leading-relaxed max-w-3xl line-clamp-2 mt-1">
                      {room.description.replace(/\n/g, ' ')}
                    </p>
                  )}

                  <div className="flex flex-wrap gap-2 mt-2">
                    <span className="bg-theme-primary/10 border border-theme-primary/20 px-2.5 py-1 rounded-[4px] text-[10px] font-black uppercase tracking-widest text-theme-primary flex items-center gap-1">
                      🛏️ {room.bed_type}
                    </span>
                    <span className="bg-theme-primary/10 border border-theme-primary/20 px-2.5 py-1 rounded-[4px] text-[10px] font-black uppercase tracking-widest text-theme-primary">
                  ...
                  {room.cancellation_policies && room.cancellation_policies.length > 0 && (
                    <div className="mt-3 bg-red-50/50 border border-red-100 p-3 rounded-lg flex flex-col gap-1.5 max-w-lg">
                      <div className="flex items-center gap-1.5 text-red-600 mb-0.5">
                        <ShieldAlert size={12} />
                        <span className="text-[10px] font-black uppercase tracking-widest">Cancellation Terms</span>
                      </div>
                      {room.cancellation_policies.map((pol: any, idx: number) => (
                        <div key={idx} className="flex justify-between items-center text-[16px] font-medium text-red-800/80 bg-red-100/30 px-2 py-1 rounded">
