...
export default function SummaryCard({ data, onNavigateTab }: SummaryCardProps) {
  ...
  const flightCount = data.flightData?.length || data.flights?.length || 0;
  const staysCount = data.stays?.length || 0;
  const toursCount = data.toursData?.length || data.tours?.length || 0;
  const attractionsList = data.attractionsData || data.attractions || [];

  // --- ROBUST WEATHER EXTRACTION ---
  const weather = data?.weather || data?.weatherData || data?.destinationInfo?.weather || {};
  
  // Safely check if weather days are in an array directly, or nested in standard API formats
  const daysArray = Array.isArray(weather) ? weather : (weather?.days || weather?.forecast?.forecastday || []);
  const firstDay = daysArray[0] || {};

  // Check all common temperature keys across different weather APIs
  const rawTemp = firstDay?.max_temp ?? 
                  firstDay?.temp ?? 
                  firstDay?.maxtemp_f ??
                  firstDay?.day?.maxtemp_f ??
  ...
  // Safely parse temperature to a rounded number
  const tempDisplay = (rawTemp !== undefined && rawTemp !== null && !isNaN(Number(rawTemp))) 
    ? Math.round(Number(rawTemp)) 
    : "--";

  // Check all common condition keys
  const condition = firstDay?.weather ?? 
                    firstDay?.conditions ??
                    firstDay?.condition?.text ??
    ...
    <div className="w-full flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="flex flex-col w-full gap-1.5 md:col-span-1">
          <label className="text-[10px] lg:text-[10px] uppercase font-black tracking-widest text-theme-secondary/60 px-4">Atmosphere</label>
          <div className="relative overflow-hidden rounded-[2rem] bg-theme-white border-[1px] border-theme-secondary/10 p-6 md:p-8 shadow-sm h-full flex flex-col justify-between group hover:border-theme-primary transition-all duration-300">
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2.5 rounded-[1rem] bg-theme-primary/5 text-theme-primary text-[10px] font-black border border-theme-primary/10 backdrop-blur-md uppercase tracking-widest w-fit">
        ...
        <div className="flex flex-col w-full gap-1.5 md:col-span-2">
          <label className="text-[10px] lg:text-[10px] uppercase font-black tracking-widest text-theme-secondary/60 px-4">About {destination.split(',')[0]}</label>
         ...
         >
           <div className="flex items-center gap-4">
             ...
             <div className="text-left flex flex-col justify-center">
               ...
               <p className="text-[10px] sm:text-[10px] font-bold text-theme-secondary/50 mt-0.5">
         ...
         <div className="mt-4 flex flex-col">
           <div className="flex justify-between items-center px-2 mb-4">
             ...
             <button 
                ...
                className="flex items-center gap-1 text-[10px] font-black text-theme-secondary/40 hover:text-theme-primary uppercase tracking-widest transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-theme-primary rounded-md px-2 py-1"
             >
                ...
                return (
                  ...
                  >
                    <div className="w-full h-40 sm:h-48 overflow-hidden bg-theme-secondary/5 relative">
                      ...
                      {attr.rating && (
                        <div className="absolute bottom-3 left-3 bg-black/60 backdrop-blur-md px-2.5 py-1.5 rounded-md text-[10px] font-bold text-white flex items-center gap-1.5">
                          ⭐ {attr.rating}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-5 flex flex-col flex-1 justify-between gap-3">
                       ...
                       <span className="text-[10px] font-black text-theme-secondary/40 uppercase tracking-widest line-clamp-1">
                         {categoryLabel}
                       </span>
                    </div>
                  </button>
