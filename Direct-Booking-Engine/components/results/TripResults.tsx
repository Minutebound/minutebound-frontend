...
import { Compass, Plane, Car, Building2, Map, ListFilter } from "lucide-react";
...
const TripResults: React.FC<TripResultsProps> = ({ data, loading, error, onOpenItinerary }) => {
  ...
  return (
    <div className="w-full relative flex flex-col min-h-screen bg-theme-surface/30">      
      
      {/* FIX 5: TABS HEADER - z-[90] (Safe to slide under SearchBar z-[100]) */}
      <div className="sticky top-[132px] z-[50] bg-theme-white/95 backdrop-blur-xl border-b border-theme-secondary/10 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] transition-all">
        
        <div className="flex justify-between items-center px-4 md:px-8 py-4 md:py-5">
          <div>
            <h1 className="text-[20px] md:text-[24px] font-black text-theme-secondary tracking-tight">
          ...
          {data && !loading && (
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 text-xs md:text-sm font-black uppercase tracking-wider rounded-xl transition-all active:scale-95 ${
                canGenerateItinerary 
                  ? "bg-theme-secondary hover:bg-theme-primary text-theme-white shadow-lg shadow-theme-secondary/20" 
        ...
        {error && (
          <div className="bg-red-50 text-red-700 p-3 border border-red-100 rounded-xl mb-2 text-sm font-bold shadow-sm mx-4 md:mx-8">
            {error}
          </div>
        )}
              ...
              {tabs.map((tab) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[14px] md:text-[15px] font-bold tracking-wide flex items-center">
                      {tab.label}
                      {badgeCount > 0 && (
                    ...
                    {isActive && (
                        <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-theme-primary rounded-t-full shadow-[0_-2px_10px_rgba(var(--theme-primary-rgb),0.3)]" />
                    )}
                  </button>
                );
              })}
      ...
      <div className="w-full p-4 md:p-6 lg:p-8 relative z-10" key={searchId}>
        {loading ? (
          <div className="mt-20">
            <Loader message="Curating your journey..." />
          </div>
        ) : !data ? (
          <div className="flex flex-col items-center justify-center py-24 md:py-32 border border-theme-secondary/10 bg-theme-white w-full rounded-[2rem] shadow-sm mt-4">
