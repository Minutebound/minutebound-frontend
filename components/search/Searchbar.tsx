...
export default function SearchBar({
  ...
  const renderFullSearchContent = () => (
    <div className="relative w-full z-30 flex flex-col items-center justify-center">
      ...
      <div className={`absolute top-0 w-full bg-gradient-to-b from-theme-secondary to-theme-secondary shadow-inner ${isCompact ? 'w-full max-w-[100%] h-full rounded-[2.5rem]' : 'h-[69%]'}`}></div>
      ...
      <div className={`relative z-10 mx-auto px-6 md:px-6 lg:px-8 py-8 lg:py-12 overflow-visible ${isCompact ? 'w-full max-w-[100%] pt-8 lg:pt-8' : 'w-[85%] max-w-[1800px]'}`}>        
        ...
        <div className="mb-4 flex flex-wrap items-center gap-4 relative z-10">
          <div className="flex bg-theme-secondary/20 p-1 rounded-full border border-theme-white/10 shadow-sm">
            <button 
        ...
        <div className="bg-theme-white w-full rounded-[2rem] shadow-[0_16px_50px_rgba(0,0,0,0.12)] p-4 md:p-5 lg:p-6 flex flex-col gap-4 relative z-[90]">
          ...
          <div className="flex">
            <div className="relative group cursor-pointer flex items-center">
              <select
                ...
                className="appearance-none  bg-theme-primary/10 text-theme-primary uppercase tracking-widest cursor-pointer outline-none py-1 pl-3 pr-8 rounded-lg border border-theme-primary/20 hover:bg-theme-primary/20 transition-all"
              >
                <option value="round-trip">Round Trip</option>
                <option value="one-way">One Way</option>
              </select>
              <ChevronDown size={18} className="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none text-theme-primary" />
            </div>
          </div>
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-end w-full gap-4 lg:gap-3 overflow-visible relative z-[100]">
            ...
            <div className="flex flex-col flex-[1.2] w-full relative z-[100]">
              <div className="flex w-full px-2 lg:px-4 mb-1.5 items-center">
                 <div className="flex-1 flex items-center gap-3">
                     <label className=" font-black uppercase tracking-widest text-theme-secondary/60">From</label>
                 </div>
                 <label className=" font-black flex-1 uppercase tracking-widest text-theme-secondary/60 pl-4 lg:pl-8">To</label>
              </div>
            ...
            <div className="flex flex-col flex-[1] w-full relative z-[90]">
              <div className="flex w-full px-2 lg:px-4 mb-1.5 items-center">
                 <label className="flex-1 uppercase  font-black tracking-widest text-theme-secondary/60">Depart</label>
                 <label className={`flex-1 uppercase  font-black tracking-widest text-theme-secondary/60 pl-2 lg:pl-4 ${tripType === 'one-way' ? 'opacity-30' : ''}`}>Return</label>
              </div>
              ...
              <div className="relative flex flex-row h-12 lg:h-14 bg-theme-white rounded-[1rem] lg:rounded-l-[1rem] border-[1.5px] border-theme-secondary/30 focus-within:border-theme-primary/50 transition-colors shadow-sm z-[90]">
                ...
                <div className="flex-1 relative flex items-center px-3 md:px-5 lg:px-5 rounded-l-[1rem] lg:rounded-l-[1rem] hover:bg-theme-secondary/5 transition-colors border-r border-theme-secondary/20 z-[60] focus-within:z-[100]">
                  <Calendar size={18} className="text-theme-primary/80 shrink-0 mr-1.5" />
                  <DatePicker
                ...
                <div className={`flex-1 relative flex items-center px-3 md:px-5 lg:px-5 rounded-r-[1rem] lg:rounded-r-[1rem] transition-colors z-[60] focus-within:z-[100] ${tripType === 'one-way' ? 'bg-theme-secondary/5 cursor-not-allowed' : 'hover:bg-theme-secondary/5'}`}>
                  <Calendar size={18} className={`shrink-0 mr-1.5 ${tripType === 'one-way' ? 'text-theme-secondary/30' : 'text-theme-primary/80'}`} />
                  <DatePicker
            ...
            <div className="flex flex-row flex-wrap sm:flex-nowrap items-center justify-between sm:justify-start gap-1 sm:gap-2 w-full xl:w-auto overflow-visible relative z-50">
              ...
              <div className="relative z-50 shrink-0" ref={travellerRef}>
                <button 
                  ...
                  className="flex items-center gap-1.5 sm:gap-2 font-bold  text-theme-secondary hover:text-theme-primary transition-colors py-1 px-1.5 sm:px-2 rounded-lg hover:bg-theme-secondary/10 whitespace-nowrap"
                >
                  <Users size={18} className="text-theme-primary/80 shrink-0" />
                  <span>{totalTravellers} <span>Guest{totalTravellers !== 1 ? 's' : ''}</span></span>
                  <ChevronDown size={18} className="text-theme-secondary/60 shrink-0" />
                </button>
              ...
              <div className="relative group cursor-pointer flex items-center gap-1 sm:gap-1.5 py-1 px-1.5 sm:px-2  rounded-lg hover:bg-theme-secondary/10 transition-colors whitespace-nowrap shrink-0">
                <span className="text-base text-theme-primary/80">{budget === 'budget' ? <DollarSign size={18}/> : <GemIcon size={18}/>}</span>
                <select 
                  value={budget} 
                ...
                <ChevronDown size={18} className="absolute right-0 sm:right-2 top-1/2 -translate-y-1/2 pointer-events-none text-theme-secondary/60" />
              </div>

              <div className="flex items-center gap-1 sm:gap-2 py-1 px-1 sm:px-2 whitespace-nowrap shrink-0 ml-auto sm:ml-0">
                <span className=" font-bold tracking-wider text-theme-secondary hidden sm:inline">Radius:</span>
                <span className=" font-bold tracking-wider text-theme-secondary sm:hidden">Rad:</span>
                ...
                <div className="flex items-center bg-theme-secondary/5 border border-theme-secondary/20 rounded-md focus-within:border-theme-primary focus-within:ring-1 focus-within:ring-theme-primary transition-all overflow-hidden">
                  ...
                  <div className="flex flex-col">
                    ...
                    >
                      <ChevronUp size={18} className="text-theme-secondary cursor-pointer" />
                    </button>
                    <button
                      type="button"
                      ...
                      className="flex items-center justify-center px-1.5 py-[1px] "
                    >
                      <ChevronDown size={18} className="text-theme-secondary cursor-pointer" />
                    </button>
                  </div>
                </div>
                <span className=" font-bold uppercase tracking-wider text-theme-secondary">mi</span>
              </div>
            </div>
 ...
 return (
    <>
      {isCompact && (
        <div className="w-full bg-theme-white py-3 px-4 md:px-6 flex flex-col md:flex-row items-center justify-center sticky top-0 shadow-sm transition-all duration-300">
          ...
          <div className="w-full flex items-center justify-center">
            ...
            >
              ...
              <div className="md:hidden bg-theme-primary text-theme-white p-2.5 rounded-full shadow-sm ml-2 shrink-0">
                <PenBoxIcon size={18} strokeWidth={2.5} />
              </div>
              ...
              <div className="hidden md:flex items-center justify-between flex-1 pl-2 pr-1">
                ...
                <div className="bg-theme-white text-theme-secondary p-2 rounded-full shadow-sm group-hover:scale-105 transition-transform ml-2 shrink-0 border border-theme-secondary/10">
                  <PenBoxIcon size={18} strokeWidth={2.5} />
                </div>
              </div>
            </button>
      ...
      {isCompact && isOverlayOpen && (
        <div className="fixed inset-0 z-[120] flex items-start justify-center pt-20 px-4 bg-black/60 backdrop-blur-sm transition-all duration-300">
...
function SbCounter({ value, min, max, onChange }: { value: number; min: number; max: number; onChange: (v: number) => void; }) {
  return (
    <div className="flex items-center gap-3 bg-theme-secondary/5 rounded-full p-1 border border-theme-secondary/20">
      <button 
        type="button" 
        className="w-8 h-8 rounded-full bg-theme-white shadow-sm text-theme-secondary hover:text-theme-primary flex items-center justify-center font-bold  disabled:opacity-40 transition-all active:scale-90" 
