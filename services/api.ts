...
export const travelApi = {
  ...
  confirmFlightPrice: async (offerId: string) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/flights/price`, {
        offer_id: offerId
      }, { headers: getAuthHeaders() }); // Keep headers if you have them, it won't hurt guest checkout
      
      return response.data;
    } catch (error: any) {
      // Catch Axios errors and extract the REAL backend message
      return { 
        error: error.response?.data?.detail || "Pricing confirmation failed. The offer may have expired." 
      };
    }
  },
  ...
  bookFlight: async (offerId: string, travelers: any[], selectedSeats: any[] = []) => {
    try {
      ...
      // Catch Axios errors and extract the REAL backend message
      return { 
        error: error.response?.data?.detail || "Booking failed. The session may have expired." 
      };
    }
  },
...
getWeather: async (dest: { lat: number; lon: number }, dates: { start?: string; end?: string }) => {
  ...
  sendBookingReceipt: async (payload: any) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/itineraries/send-receipt`, payload);
      return response.data;
    } catch (error) {
  ...
  // Add this inside your exported travelApi object in services/api.ts:

  getBookingByMbRef: async (mbRef: string) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/bookings/mb-reference/${mbRef}`, {
        headers: getAuthHeaders() // Make sure you include your auth headers!
      });
      return response.data;
    } catch (error: any) {
      console.error("Error fetching booking by MB Ref:", error);
      return { 
