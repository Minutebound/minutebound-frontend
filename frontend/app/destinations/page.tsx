...
function DestinationsContent() {
  ...
  useEffect(() => {
    ...
    if (!loading && destinations.length > 0) {
      destinations.forEach((dest) => {
        ...
        el.className =
          'bg-theme-bg border border-theme-text/20 shadow-md rounded-full px-3 py-1 text-sm font-bold text-theme-text cursor-pointer transition-transform hover:scale-110 hover:bg-theme-primary hover:text-theme-bg hover:border-theme-primary';
  ...
  return (
    <div className="w-full h-[calc(100vh)] relative bg-theme-bg overflow-hidden flex flex-col md:flex-row">
      ...
      <div className="w-full md:w-[60%] lg:w-[60%] flex flex-col h-full border-r border-theme-text/10">

        {/* Sticky Header & Filter Pills */}
        <div className="p-6 pb-4 border-b border-theme-text/10 z-10 bg-theme-bg">
              ...
              <button
                ...
                className={`whitespace-nowrap px-4 py-2 rounded-full text-sm font-medium transition-colors border ${
                  activeCategory === cat
                    ...
                    : 'bg-theme-bg text-theme-text border-theme-text/20 hover:border-theme-text hover:bg-theme-bg'
                }`}
              >
                {cat}
              </button>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-theme-bg/50">
          {loading ? (
            ...
            <div className="flex justify-center items-center h-32">
              <p className="text-theme-text/50 text-sm">No destinations found.</p>
            </div>
          ) : (
            <div className="flex flex-col gap-5">
              {destinations.map((dest) => (
                <div key={dest.id} className="group flex flex-col sm:flex-row bg-theme-bg border border-theme-text/10 rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-theme-primary/5 hover:border-theme-primary/30 transition-all cursor-pointer">
                  {/* Image */}
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <p className="text-sm text-theme-text/70 mt-2 line-clamp-2">
                        {dest.description}
                      </p>
                    </div>

                    {/* Pricing Footer */}
                    <div className="mt-4 pt-4 border-t border-theme-text/10 flex items-center justify-between">
                      <div className="flex flex-col gap-0.5">
                        {dest.avg_flight_price && (
                          <span className="text-sm text-theme-text/70">
                            Flights from{' '}
                            <span className="font-bold text-theme-text">${dest.avg_flight_price}</span>
                          </span>
                        )}
                        {dest.avg_hotel_price && (
                          <span className="text-sm text-theme-text/70">
                            Hotels from{' '}
                            <span className="font-bold text-theme-text">${dest.avg_hotel_price}</span>
                          </span>
                        )}
                      </div>
      ...
      <div className="hidden md:block w-full md:w-[40%] lg:w-[40%] bg-theme-bg relative">
...
export default function DestinationsExplorer() {
  return (
    <Suspense fallback={<div className="p-8 text-center text-theme-text">Loading Explorer…</div>}>
      <DestinationsContent />
    </Suspense>
  );
}