...
export default function LandingPage() {
  ...
  return (
    <div className="w-full min-h-screen relative bg-theme-bg overflow-x-hidden">
      {/* Texture */}
          ...
          {loading ? (
            ...
            <div className="flex overflow-x-auto gap-6 pb-6 custom-scrollbar snap-x">
              {destinations.map((dest) => (
                <Link href={`/destinations?category=${encodeURIComponent(dest.category)}`} key={dest.id} className="snap-start min-w-[280px] w-[280px] group relative aspect-[4/5] rounded-[2rem] overflow-hidden bg-theme-surface/20 border border-theme-text/5 shadow-sm hover:shadow-xl transition-all cursor-pointer flex-shrink-0">
              ...
              {events.map((event) => (
                <Link href={`/events?category=${encodeURIComponent(event.category)}`} key={event.id} className="snap-start min-w-[320px] w-[320px] flex flex-col bg-theme-surface/20 border border-theme-text/10 rounded-[2rem] overflow-hidden shadow-sm hover:shadow-xl hover:border-theme-orange/30 transition-all cursor-pointer group flex-shrink-0">
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    <h3 className="font-black text-theme-text text-xl leading-tight mb-2 line-clamp-2">{event.title}</h3>
                    <div className="flex items-center gap-1.5 text-theme-muted text-xs font-bold mt-auto pt-4 border-t border-theme-text/5">
                      <MapPin size={14} />
